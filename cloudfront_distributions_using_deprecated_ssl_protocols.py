#!/usr/bin/env python3
"""
Control: CloudFront Distribution Does Not Use SSLv3, TLSv1, or TLSv1.1
for Origin Connections
------------------------------------------------------------------------
CloudFront is a global service (single API endpoint - us-east-1).
OriginSslProtocols is a field inside CustomOriginConfig only - it lists
which SSL/TLS protocol versions CloudFront is allowed to use when
connecting to that origin (SSLv3, TLSv1, TLSv1.1, TLSv1.2).

S3 origins (S3OriginConfig) have no such field - AWS manages TLS
negotiation to S3 internally, so this control is not applicable to them.

Per distribution:
  - No CustomOriginConfig origins at all      -> SKIPPED (not applicable)
  - All custom origins allow only TLSv1.2      -> COMPLIANT
  - Any custom origin still allows SSLv3/TLSv1/TLSv1.1 -> NON_COMPLIANT
    (weak protocol(s) and origin named explicitly in evidence)
"""
...
CONTROL_NAME = (
    "CloudFront Distribution Does Not Use SSLv3, TLSv1, or TLSv1.1 "
    "for Origin Connections"
)
...
def evaluate_origin_ssl_protocols(dist: dict):
    """
    Returns (status, evidence). Only CustomOriginConfig origins are
    evaluated; S3OriginConfig origins are excluded since the field does
    not apply to them.
    """
    ...
    for origin in custom_origins:
        ...
        weak_found = sorted(set(ssl_protocols) & WEAK_PROTOCOLS)

        if weak_found:
            violations.append(f"{origin_id} (allows: {', '.join(weak_found)})")
        else:
            compliant_origins.append(origin_id)

    if violations:
        return (
            "NON_COMPLIANT",
            f"{len(violations)}/{len(custom_origins)} custom origin(s) still allow "
            f"weak TLS/SSL protocols: {', '.join(violations)}"
        )

    return (
        "COMPLIANT",
        f"All {len(custom_origins)} custom origin(s) restrict to TLSv1.2 only: "
...
def check_control(session):
    ...
    for dist in tqdm(distributions, desc="Scanning CloudFront Distributions"):
        ...
        status, evidence = evaluate_origin_ssl_protocols(dist)
...
def write_csv(results, account_id):
    filename = f"cloudfront_origin_ssl_protocols_{account_id}.csv"
